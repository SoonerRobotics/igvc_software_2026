...
uint8_t robot_voltage = 255; // 0.1V increments, 255 = no data received yet
...
void updateDisplay() {
  ...
  if (is_connected) {
    ...
    } else if (robot_voltage != 255) {
        // Print robot voltage in volts on line 2 with 1 decimal place
        // accounting for spacing of 10.0V vs 9.5V
        lcd.setCursor(0, 1);
        lcd.print("Robot: ");
        if (robot_voltage < 100) {
          lcd.print(" ");
        }
        lcd.print((float)robot_voltage / 10.0f, 1);
        lcd.print("V");
    }
  } else {
    lcd.print("Waiting...");
  ...
  if (rf95.available())
  {
    ...
    if (rf95.recv(buf, &len))
    {
      ...
      if (strcmp(GLOBAL_PASSWORD, incoming_message.password) == 0) {
        ...
        if (incoming_message.id == MSG_HEARTBEAT_REPLY_ID) {
            robot_voltage = incoming_message.signal;
        }
      }
    }
  }
