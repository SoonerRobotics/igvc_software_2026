...
void writeFrame(ShmContext &ctx, const cv::Mat &frame, uint32_t seq) {
    ...
    if (!semTimedWait(ctx.sem)) return;
...
int main() {
    ...
    std::thread leftThread([&]()  { runCamera("/dev/video2", leftShm,  "left",  12, g_running); });
    std::thread rightThread([&]() { runCamera("/dev/video0", rightShm, "right", 12, g_running); });
    ...
    closeShm(leftShm);
    closeShm(rightShm);
    shm_unlink(SHM_LEFT_NAME);  sem_unlink(SEM_LEFT_NAME);
    shm_unlink(SHM_RIGHT_NAME); sem_unlink(SEM_RIGHT_NAME);
    return 0;
}